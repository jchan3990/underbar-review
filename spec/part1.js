  ...
  describe('Part I', function() {
    ...
    describe('indexOf', function() {
      ...
      it('returns -1 when the target cannot be found not in the list', function() {
        var numbers = [10, 23, 45, 50];

        expect(_.indexOf(numbers, 35)).to.equal(-1);
      });

      it('returns the first index that the target can be found at when there are multiple matches', function() {
        var numbers = [10, 15, 15, 15, 15, 20, 40, 15];
        expect(_.indexOf(numbers, 15)).to.equal(1);
      });
    });
    ...
    describe('map', function() {
      ...
      it('should apply a function to every value in an array', function() {
        var multiplyByTwo = _.map([1, 2, 3], function(x) {return x * 2});

        expect(multiplyByTwo).to.eql([2, 4, 6]);
      });
    ...
    describe('reduce', function() {
      ...
      it('should accept falsy as a valid memo', function() {
        var result = _.reduce([1, 2, 3], function(memo, item) {
          return memo * item;
        }, 0);
